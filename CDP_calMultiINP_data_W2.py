...
# Author  : xuzhou
# Date    : 2019-07-15
# Function: 计算砖和砂浆和UHPC的CDP模型材料参数。
...
# ==================================================================================================
outputDir = "E:/Temp/Wall/W2-Test/outputInp"         #输出的inp文件目录
caeFullPath = "E:/Temp/Wall/W2-Test/modelchange.cae" #获得cae的完整路径
...
#从csv文件中读取工况数据
fileName = "C:/Users/ASUS/Desktop/fc_Brick_Lime_UHPC.csv"     #指定文件路径
fc_Brick_Lime = []                                            #初始化数组
...
for line in fstr[1:]:                                         #循环文件中各行，排除第一行
    ...
    if len(words)==9:                                         #当列表长度为9时，表示有数据
        fc_Brick_Lime.append([float(words[0]),float(words[1]),float(words[2]),float(words[3]),float(words[4]),
        float(words[5]),float(words[6]),float(words[7]),float(words[8])])   #存储数据到列表
...
#UHPC材料参数输入
den_uhpc = 2500                 #密度
elas_uhpc = 38591e6             #弹性模量
pos_uhpc = 0.2                  #泊松比
fc_uhpc = 96e6                  #抗压强度
ft_uhpc = 4.5e6                 #抗拉强度
ec0_uhpc = 0.0035               #极限压应力对应的应变
ecp_uhpc = 0.015                #取值最大压应变
p1_uhpc = 30                    #CDP模型中的 膨胀角
p2_uhpc = 0.1                   #CDP模型中的 偏心率
p3_uhpc = 1.16                  #CDP模型中的 双向/单向抗压强度比
p4_uhpc = 0.6667                #CDP模型中的 不变应力比
p5_uhpc = 0.005                 #CDP模型中的 粘聚系数
wt_uhpc = 1                     #拉伸恢复系数
wc_uhpc = 0                     #压缩恢复系数
fcInitial_uhpc = 0.7            #初始损伤的压应力比率
dfct_uhpc = 0.05                #单个增量步运行的压应力变化比例（控制输出点间距）
maxDamage_uhpc = 0.9            #最大损伤阈值
...
class xuzhouCDP_UHPC(xuzhouCDP_Brick):
    def createMaterial(self,matName,den,elas,pos,fc,ft,p1,p2,p3,p4,p5,wt,wc,
    fcInitial,dfct,maxDamage,ec0,ecp,modelName):
        self.matName = matName          #材料名称
        self.den = den                  #密度
    ...
    def cal_Plastic(self):
        ...
        yc = [0]
        dx = 0                                                          #前期线性段引起应力应变曲线左移量
        e_sec = self.fc/self.ec0
        if(self.elas<=e_sec):
            getWarningReply(message="Error1 : UHPC Data Error!",buttons=(CANCEL,))
            return 0
        r = self.elas/(self.elas-e_sec)
        x1 = np.array(range(1001))/1000.0                               #归一化应变
        y = x1*r/(r-1+x1**r)                                            #归一化应力
        checkElas_c = self.ec0/(self.fc/self.elas)                      #初始模量的归一化变换
        for i in range(len(y)):
            if y[i]>=self.fcInitial:                                    #当应力大于初始塑性应力
                if len(xc)==1:                                          #当初始屈服
                    ...
                    xc.append(x1[i]-dx)                                 #记录为初始屈服点应变
                    yc.append(y[i])                                     #记录为初始屈服点应力
                elif abs(y[i]-yc[-1])>=self.dfct and 1-y[i]>self.dfct/2.0:   #当后屈服点大于前屈服点self.dfc，且小于1-self.dfc/2.0时
                    xc.append(x1[i]-dx)                                 #添加记录点应变
                    yc.append(y[i])                                     #添加记录点应力
                    lineElas = (yc[-1]-yc[-2])/(xc[-1]-xc[-2])          #前后两个点计算切线模量
        xc.append(1-dx)                                                 #记录峰值应变
        yc.append(1)                                                    #记录峰值应力
        x2 = 1+np.array(range(1,int(((self.ecp)/self.ec0-1+dx)*1000)))/1000.
        y = x2*r/(r-1+x2**r)                                            #归一化应力
        for i in xrange(len(y)):
            if yc[-1]-y[i]>=self.dfct:
                xc.append(x2[i]-dx)
                yc.append(y[i])
        ...
        self.inpc[1]=0              #初始屈服点对应的非弹性应变也为0
        #========================拉应力-应变曲线====================
        self.xt = [0,self.ft/self.elas,self.etp]                                    #总拉应变
        self.yt = [0,self.ft,0.1*self.ft]                                           #总拉应力
        self.inpt = [self.xt[i]-self.yt[i]/self.elas for i in range(len(self.xt))]  #非弹性拉应变
        self.inpt[0]=0              #初始非弹性应变为0
        self.inpt[1]=0              #初始屈服点对应的非弹性应变也为0
        print "cal_Plastic_UHPC"
        return 1
...
if len(mdb.models.keys())==1:
    for fci,fc in enumerate(fc_Brick_Lime):
        ...
        fc_uhpc = fc[2]                                 #UHPC抗压强度
        ft_uhpc = fc[3]                                 #UHPC抗拉强度
        elas_mortar = fc[4]                             #砂浆弹性模量
        ec0_brick = fc[5]                               #砖极限压应力对应的应变
        ecp_brick = fc[6]                               #砖最大压应变
        ec0_mortar = fc[7]                              #砂浆极限压应力对应的应变
        ecp_mortar = fc[8]                              #砂浆最大压应变
        ...
        uhpcMain = xuzhouCDP_UHPC()
        out3 = uhpcMain.createMaterial(matName_uhpc,den_uhpc,elas_uhpc,pos_uhpc,
            fc_uhpc,ft_uhpc,p1_uhpc,p2_uhpc,p3_uhpc,p4_uhpc,p5_uhpc,wt_uhpc,wc_uhpc,
            fcInitial_uhpc,dfct_uhpc,maxDamage_uhpc,ec0_uhpc,ecp_uhpc,modelName)
        #创建任务并输出inp
        jobName = "%d-%s_B%.2f_L%.5f_U%.2f"%(fci,modelName,fc_brick/1000000.0,ft_mortar/1000000.0,fc_uhpc/1000000.0) #任务名称为cae名称+model名称
...
starttime = time.time()
# 目录文件:根据inp文件路径进行修改
root_dir = "E:/Temp/Wall/W2-Test/outputInp/"
timeFile = root_dir+"time.csv"
cpuNum = 4      #每个任务占用的cpu数量
...
import customKernel
#获得指定目录下的odb文件名称列表
odbFilePath = "E:/Temp/Wall/W2-Test/outputInp/"                   #需要修改的目录名称，名称后面注意有个反斜杠
outPutPath = "E:/Temp/Wall/W2-Test/outputInp/result/"             #指定输出目录
